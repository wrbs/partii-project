...
The optimising compiler operates on the level of a single function. It starts by performing a
...
.1 /.
.2 benchmarks.
.3 sandmark\DTcomment{\textbf{fork}: benchmark suite I significantly modified}.
.3 analysis\DTcomment{\textbf{mine}: Jupyter notebooks analysing benchmark results}.
.2 docs\DTcomment{\textbf{mine}: {\LaTeX} source of proposal, report and this document}.
.2 scripts\DTcomment{\textbf{mine}: scripts to run tests and graph basic blocks}.
.2 src.
.3 rust.
.4 ocaml-jit-shared\DTcomment{\textbf{mine}: shared library between the other two crates}.
.4 ocaml-jit-staticlib\DTcomment{\textbf{mine}: crate linked in to OCaml runtime}.
.4 ocaml-jit-tools\DTcomment{\textbf{mine}: binary crate with standalone development/testing
      tools}.
...
.2 test-programs\DTcomment{\textbf{mine/vendor}: OCaml source for the small test programs
      used by the scripts}.
