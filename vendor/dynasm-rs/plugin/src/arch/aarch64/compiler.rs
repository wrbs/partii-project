        ...
        match *arg {
            FlatArg::Direct {
                ...
                Command::RNext => {
                    ...
                    }) = data.args.get(cursor - 1)
                    {
                        match prevreg {
                            RegKind::Static(previd) => {
                                if id.code() != ((previd.code() + 1) % 32) {
                                    emit_error!(
                                        span,
                                        "Invalid register. This register has to be the register after the previous argument."
                                    );
                                    return Err(None);
                                }
                            }
                            RegKind::Dynamic(_, _) => {
                                if id != RegId::XZR {
                                    emit_error!(
                                        span,
                                        "Please use XZR here to indicate that it should be the register after the previous argument."
                                    );
                                    return Err(None);
                                }
                            }
                        ...
                        panic!(
                            "RNext command without the previous command being a register encoder"
                        );
                    }
                }
            ...
            FlatArg::Direct {
                ...
                Command::RNext => {
                    emit_error!(
                        span,
                        "This register is constrained to be the register after the previous argument's register. As such, it does not support dynamic registers. Please substitute it with XZR to indicate this"
                    );
                    return Err(None);
                }
                _ => panic!("Invalid argument processor"),
